# 熙记 (Xiji) - 下一代协作式 AI 笔记平台

[English](./README_EN.md) | [简体中文](./README.md)

**熙记** 是一个基于 **Next.js 14 (App Router)** 构建的现代化笔记应用，旨在融合**本地优先 (Local-First)** 的流畅体验与 **实时协作** 的强大功能。它不仅是一个 Markdown 编辑器，更集成了 AI 辅助写作、心愿追踪系统以及独特的“回忆胶囊”功能。

本项目重点展示了在**复杂状态管理**、**实时数据同步**、**离线支持**以及**富文本编辑器定制**方面的技术深度。

## ✨ 核心特性

- **📝 强大的富文本编辑器**: 基于 Plate.js (Slate) 构建，支持 Markdown 语法、快捷指令 (Slash Command)、代码块高亮及媒体嵌入。
- **🤝 实时协同编辑**: 多人同时编辑同一文档，光标实时可见，毫秒级同步 (Powered by Liveblocks & Yjs)。
- **⚡️ 本地优先 & 离线支持**: 采用 IndexedDB 进行本地存储，断网状态下依然可用，网络恢复后自动通过同步队列 (Sync Queue) 同步数据。
- **🤖 AI 智能辅助**: 采用混合 AI 架构——利用 Vercel AI SDK 实现 Copilot 风格的自动续写，并结合自定义 OpenAI/DeepSeek 集成实现润色、总结等高级指令。
- **📂 灵活的组织结构**: 支持无限层级文件夹与多标签 (Many-to-Many) 分类系统。
- **🌟 心愿与回忆**: 独特的产品功能，支持进度追踪的心愿单与基于时间的“回忆胶囊”。
